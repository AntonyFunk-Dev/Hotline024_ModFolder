...
function onCreatePost() {
	if (cam[2].pos_locked != null && cam[2].pos_locked.length > 0) {
		FlxG.camera.scroll.x = (cam[2].pos_locked[0] ?? 0) - FlxG.camera.width / 2;
		FlxG.camera.scroll.y = (cam[2].pos_locked[1] ?? 0) - FlxG.camera.height / 2;
	}
...
function onUpdatePost() {
	...
	if (game.gf != null && gfSection) {
		if (game.gf.getAnimationName().indexOf('sing') == -1) FlxG.camera.targetOffset.set(0, 0);

		return if (getVar('updateCamera')) setMoveCamera(2);
	}

	if (!mustHitSection) {
		if (game.dad.getAnimationName().indexOf('sing') == -1) FlxG.camera.targetOffset.set(0, 0);

		if (getVar('updateCamera')) setMoveCamera(1);
	} else {
		if (game.boyfriend.getAnimationName().indexOf('sing') == -1) FlxG.camera.targetOffset.set(0, 0);
...
function goodNoteHit(note) {
	...
	if (mustHitSection) {
		if (game.gf != null && gfSection) {
			moveOffsetCamera(2, note);
		} else {
			moveOffsetCamera(0, note);
		}
	}
...
function onEvent(name, value1, value2) {
	if (name == 'Camera Follow Pos') {
		isCameraOnForcedPos = !Math.isNaN(Std.parseFloat(value1));
	}
...
function setMoveCamera(?charType:Int = 0) {
	if (cam[charType].pos_locked != null && cam[charType].pos_locked.length > 0) {
		game.camFollow.x = cam[charType].pos_locked[0] ?? 0;
		game.camFollow.y = cam[charType].pos_locked[1] ?? 0;
	} else {
		switch (charType) {
			case 2:
...
function moveOffsetCamera(?charType:Int = 0, ?note:Note = null) {
	...
	FlxG.camera.targetOffset.x = (note.noteData == 0 ? -cam[charType].target_offset ?? 0 : (note.noteData == 3 ? cam[charType].target_offset ?? 0 : 0));
	FlxG.camera.targetOffset.y = (note.noteData == 1 ? cam[charType].target_offset ?? 0 : (note.noteData == 2 ? -cam[charType].target_offset ?? 0 : 0));
}
