...
function spawnParticle() {
	for (part in partsGroup.members) {
		if (!part.visible) {
			...
			if (twnArray[partsGroup.members.indexOf(part)] != null && twnArray[partsGroup.members.indexOf(part)].active) twnArray[partsGroup.members.indexOf(part)].cancel();
			twnArray[partsGroup.members.indexOf(part)] = FlxTween.tween(part.scale, {x: 0, y: 0}, FlxG.random.float(1, 5), {startDelay: (Conductor.crochet / 1000) * FlxG.random.int(5, 15)});
